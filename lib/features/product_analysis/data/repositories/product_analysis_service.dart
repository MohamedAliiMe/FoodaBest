  ...
  Future<List<ProductModel>> searchAlternativeProducts(
    ...
    try {
      ...
      for (final ingredient in ingredients) {
        ...
        try {
          // Build parameter map for search with allergen exclusions
          ...
          if (userProfile != null && userProfile.displayName != null) {
            ...
            // Open Food Facts allergen filtering approaches:
            // 1. Use WithoutAdditives parameter for common allergens
            // 2. Use TagFilter for specific allergen exclusions
            // 3. Use AllergensFilter for comprehensive allergen filtering
            //*^
            // for (final allergy in userProfile.allergens ?? []) {
            //   final allergyLower = allergy.toLowerCase().trim();

            //   // Map common allergens to Open Food Facts tags
            //   String? allergenTag = _mapAllergenToTag(allergyLower);

            //   if (allergenTag != null) {
            //     // Use TagFilter to exclude products with this allergen
            //     parameters.add(TagFilter.fromType(
            //       tagFilterType: TagFilterType.ALLERGENS,
            //       tagName: allergenTag,
            //       contains: false, // Exclude products containing this allergen
            //     ));
            //     log('Added allergen filter: $allergenTag');
            //   } else {
            //     // For unmapped allergens, use a general approach
            //     log('Warning: Could not map allergen "$allergy" to Open Food Facts tag');
            //   }
            // }
          }
