import 'package:flutter/material.dart';

class AllColors {
  static const Color black = Color(0xff000000);
  static const Color white = Color(0xffF9F9F9);
  static const Color whiteBase = Color(0xffFFFFFF);
  static const Color grey = Color(0xff8E8E8E);
  static const Color grayLight = Color(0xffB3B3B3);
  static const Color red = Color(0xFFD12B2B);
  static const Color blue = Color(0xff6B4EFF);
  static Color blueBackground = const Color(0xff7582AE).withValues(alpha: 0.3);

  static const Color darkBlue = Color(0xffBCBCBC);
  static const Color yellow = Color(0xffF5BA00);
  static const Color yellowLight = Color(0xffFFE8C5);
  static const Color green = Color(0xff2B8761);
  static const Color transparent = Color(0x00000000);
  static const Color home1Container = Color(0xffFFE1B4);
  static const Color customFavoriteColor = Color(0xffFECF08);
  static const Color home2Container = Color(0xffFF9F06);
}
